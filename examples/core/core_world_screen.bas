...
while not WindowShouldClose()
  ...
  BeginDrawing()
    ...
    DrawText("Enemy: 100 / 100", clng(cubeScreenPosition.x) - MeasureText("Enemy: 100/100", 20)/2, clng(cubeScreenPosition.y), 20, BLACK)
    
    DrawText(TextFormat("Cube position in screen space coordinates: [%i, %i]", clng(cubeScreenPosition.x), clng(cubeScreenPosition.y)), 10, 10, 20, LIME)
